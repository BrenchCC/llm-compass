...
**验证择优接受**是整个机制的闸门：每个候选技能都在一个留出的 selection split 上评测。论文规则原文是——若候选分高于当前 selection 分，则它成为新的当前技能；若还超过历史最佳，则写入 `best_skill.md`；否则**拒绝**。形式化地说，设 $S(\cdot)$ 为留出验证分，候选 $s'$ 仅当 $S(s') \gt  S(s_{\text{cur}})$ 时被接受，这与"只有降低验证损失才更新"的优化器纪律同构。
